...
export function parseModuleGraph(bin, preRead = null) {
  ...
  try {
    ...
    let from = Math.max(bun.off, winEnd - TRAILER_WINDOW)
    let magicPos = reader.lastIndexOf(MAGIC, from, winEnd)
    if (magicPos < 0 && from > bun.off) {
      magicPos = reader.lastIndexOf(MAGIC, bun.off, winEnd) // fail-safe：扩到全 .bun 窗，宁慢不静默错
    }
    if (magicPos < bun.off) throw new Error('parseModuleGraph: Bun trailer not found in .bun window')
    const trailerOffset = magicPos - 1 // 指向 trailer 前导 '\n'
...
function decodeRecords(reader, trailerOffset, bun) {
  ...
  // 真正有风险的是上界越界。
  if (recordsArrayStart + mpLen > fileSize) {
    throw new Error(`decodeRecords: records array [${recordsArrayStart}, ${recordsArrayStart + mpLen}) out of bounds (file ${fileSize})`)
  }
  ...
  for (let i = 0; i < n; i++) {
    ...
    // 边界：name 与 contents 都必须落在文件内。nameOff/cOff 是 readUInt32LE（无符号恒 ≥0），
    // 故不做 `< 0` 检查——真正的下界（nameAbs≥graphBase≥bun.off）已由 graphBase 校验传递覆盖。
    if (nameAbs + nameLen > fileSize) {
      throw new Error(`decodeRecords: record #${i} name [${nameAbs}, ${nameAbs + nameLen}) out of bounds`)
    }
    if (offset + cLen > fileSize) {
      throw new Error(`decodeRecords: record #${i} contents [${offset}, ${offset + cLen}) out of bounds`)
    }
