...
function normalizeSubstates(feature, windows) {
  ...
  for (const record of records) totals.set(record.id, (totals.get(record.id) ?? 0) + 1)
  return records.map((record) => {
    const kind = record.id
    const index = indexes.get(kind) ?? 0
    indexes.set(kind, index + 1)
    return { ...record, id: totals.get(kind) === 1 ? kind : `${kind}:${index}` }
  })
}

function candidateRanges(feature, discoveryWindows, size) {
  if (!CLAUDE_FEATURES.has(feature.name)) return null
  return mergeRanges(normalizeSubstates(feature, discoveryWindows).map((record) => [
  ...
  try {
    ...
    for (const feature of features) {
      ...
      if (detected == null || detected.state === 'unsupported') {
        ...
        statuses[feature.name] = featureStatus(detected, normalizeSubstates(feature, windows))
      }
