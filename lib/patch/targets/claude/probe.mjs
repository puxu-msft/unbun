...
// fail-closed 的核心：候选区间必须完整落在已读的 discovery 窗内才敢只看窗口，否则回落 full
// detect。曾另有一条「source-exec 候选不得跨越 32MB discovery 边界」的规则，随 source-exec 改用
// 全文件锚点 census 而失效——已经没有 discovery 边界可跨了。
function candidatesComplete(feature, ranges, discoveryWindows) {
  return ranges.every(([start, end]) => discoveryWindows.some(
...
export function probeClaudeBinary(path, options = {}) {
  ...
  try {
    ...
    for (const feature of features) {
      ...
      const complete = candidates == null || candidates.length > 0 && candidatesComplete(feature, candidates, discoveryWindows)
