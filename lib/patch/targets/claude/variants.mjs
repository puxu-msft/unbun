...
export function validateReplay(currentSites, desiredSites, { allowAbsent = false } = {}) {
  ...
  for (let index = 0; index < currentSites.length; index++) {
    ...
    if (allowAbsent && desired.state === ABSENT) {
      ...
      // 占位不对应任何字节（replay 直接跳过它），其 offset 取自 `bytes.length`，会随输入长度
      // 漂移：窗口化观测得到窗口末端，full detect 得到整个二进制长度。这个数值**无语义**，
      // 不能参与相等校验，否则窗口化与 full 的观测互不兼容，会阻断所有 feature 的写入。
      continue
    }
    if (desired.offset !== current.offset || desired.length !== current.length) {
      throw substateError(`site identity mismatch at index ${index}`)
    }
