...
function replaySubstates(bytes, desired, options) {
  const output = writable(bytes, options)
  const located = locate(output)
  const current = recordsFromSites(located.sites)
  validateReplay(current, desired)
  for (let index = 0; index < current.length; index++) {
    const site = located.sites[index]
    ...
    if (desired[index].state !== site.state) replacementFor(site, desired[index].state, output).copy(output, site.localOffset)
  }
