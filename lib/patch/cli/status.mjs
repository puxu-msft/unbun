...
export async function readStatus(binaryPath, { profile = false } = {}) {
  ...
  // L3B-01：探测必须针对**实际会被写入的对象**（canonical/realpath），否则在 symlink 布局下
  // status 报告的是 symlink 的状态，而 patch/revert 作用于其目标，两者可能完全不同。
  const probe = probeClaudeBinary(context.binary)
