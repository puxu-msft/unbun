...
export function outdirName(bin, version) {
  ...
  if (version) {
    if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(version) || version.includes('..')) {
      throw new Error(`outdirName: Claude version must be a safe semver filename segment: ${JSON.stringify(version)}`)
    }
    segment = version
  } else {
    segment = basename(bin)
    if (!segment || segment === '.' || segment === '..' || /[\\/\x00-\x1f\x7f]/.test(segment)) {
      throw new Error(`outdirName: binary basename is not a safe filename segment: ${JSON.stringify(segment)}`)
    }
  }
  return `claude-code-${segment}`
}

export function refsOutdir(refsRoot, ...segments) {
  const root = resolve(refsRoot)
  const out = resolve(root, ...segments)
  const rel = relative(root, out)
  if (rel === '..' || rel.startsWith(`..${sep}`)) {
    throw new Error(`refsOutdir: resolved path is outside refs root: ${out}`)
  }
  return out
}
