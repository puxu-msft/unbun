...
export const CC_PAYLOAD = 'process.env.CC_EXT&&require(process.env.CC_EXT)'
...
export function patchLoaderHook(buf, { force = false, anchor = CC_ANCHOR, payload = CC_PAYLOAD } = {}) {
  ...
  if (typeof anchor !== 'string' || anchor.length === 0) throw new Error('anchor must be a non-empty latin1 string')
  if (typeof payload !== 'string' || payload.length === 0) throw new Error('payload must be a non-empty latin1 string')
  const anchorBuf = Buffer.from(anchor, 'latin1')
  const rawPayloadBuf = Buffer.from(payload, 'latin1')
  if (anchorBuf.toString('latin1') !== anchor || rawPayloadBuf.toString('latin1') !== payload) {
    throw new Error('anchor and payload must round-trip losslessly through latin1')
  }
  if (rawPayloadBuf.length > anchorBuf.length) {
    throw new Error(`payload (${rawPayloadBuf.length} bytes) longer than anchor (${anchorBuf.length} bytes); cannot keep equal length`)
  }
  const len = anchorBuf.length
  const payloadBuf = Buffer.alloc(len, 0x20)
  rawPayloadBuf.copy(payloadBuf)
